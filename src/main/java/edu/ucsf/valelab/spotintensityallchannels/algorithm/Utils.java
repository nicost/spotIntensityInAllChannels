...
import edu.ucsf.valelab.spotintensityallchannels.utils.ListUtils;
import ij.ImagePlus;
 ...
 */
public class Utils {
   private static QuartFilledCircleMask circleMask_;
   private static QuartCircleOutlineMask circleOutlineMask_;
   ...
   public static float GetAvgIntensity(ImageProcessor ip, int x, int y, int radius) {
      ...
      if (circleMask_ == null || circleMask_.getRadius() != radius)
         circleMask_ = new QuartFilledCircleMask(radius);
      //circleMask_.print();
   ...
   public static float GetBackgroundCircleRp2(ImageProcessor ip, int x, int y, int radius) {
      ...
      if (ip instanceof FloatProcessor) {
         for (int i = 0; i <= radius; i++) {
            for (int j = 0; j <= radius; j++) {
               if (circleOutlineMask_.getMask()[i][j]) {
                  if (i == 0 && j == 0) {
                     results.add( ip.getf(x, y));
                  }
                  else if (i == 0) {
                     results.add(ip.getf(x, y + j));
                     results.add(ip.getf(x, y - j));
                  }
                  else if (j == 0) {
                     results.add(ip.getf(x + i, y));
                     results.add(ip.getf(x - i, y));
                  }
                  else {
                     results.add(ip.getf(x - i, y - j));
                     results.add(ip.getf(x - i, y + j));
                     results.add(ip.getf(x + i, y - j));
                     results.add(ip.getf(x + i, y + j));
                  }
         ...
         for (int i = 0; i <= radius; i++) {
            for (int j = 0; j <= radius; j++) {
               if (circleOutlineMask_.getMask()[i][j]) {
                  if (i == 0 && j == 0) {
                     results.add((float) ip.get(x, y));
                  }
                  else if (i == 0) {
                     results.add((float) ip.get(x, y + j));
                     results.add((float) ip.get(x, y - j));
                  }
                  else if (j == 0) {
                     results.add((float) ip.get(x + i, y));
                     results.add((float) ip.get(x - i, y));
                  }
                  else {
                     results.add((float) ip.get(x - i, y - j));
                     results.add((float) ip.get(x - i, y + j));
                     results.add((float) ip.get(x + i, y - j));
                     results.add((float) ip.get(x + i, y + j));
                  }
      ...
      return ListUtils.median(results );
   }
